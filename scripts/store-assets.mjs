...
const root = path.resolve(import.meta.dirname, ".."),
  out = path.join(root, "marketing/output");
...
const related = [
  tab("notion.so", "Studio refresh · Project overview", "studio-refresh"),
  tab("notion.so", "Studio refresh · Materials & finishes", "materials"),
  tab("notion.so", "Studio refresh · The reading corner", "reading-corner"),
  tab("notion.so", "Studio refresh · Ideas to explore", "ideas"),
];
const duplicates = [
...
await page.addInitScript((data) => {
  ...
  Object.defineProperty(globalThis, "LanguageModel", {
    value: {
      ...
      create: async () => ({
        prompt: async (prompt) =>
          prompt.includes("UNTRUSTED_GROUP_METADATA_JSON")
            ? "Studio refresh"
            : "1",
        destroy() {},
      }),
