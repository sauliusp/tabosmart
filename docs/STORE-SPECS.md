...
For the current 0.3.0 campaign, use [the artifact index](../ARTIFACTS.md) and [current listing copy](STORE-LISTING.md). All five current screenshots and both promotional tiles are in `marketing/community-launch/store/`. The historical paths below refer to earlier local assets, which are not included in the public repository.
