...
On 9 September 2026, all 366 unit tests and source checks passed. The unpacked Chromium suite passed installation-guide, favicon, review/action, responsive and accessibility checks. Website TypeScript checks and the production build passed. GitHub Actions repeats these checks on each PR revision. This does not establish real local-model inference or Store approval.

Run `npm ci`, `npm test`, `npm run check`, `npx playwright install chromium`, `npm run qa:browser`, `npm run qa:workspace` and `npm run pack`. Run `npm ci --prefix website` and `npm run build --prefix website` for the site. Generated reports are local and excluded from Git. The current package and campaign index is [ARTIFACTS.md](../ARTIFACTS.md).
